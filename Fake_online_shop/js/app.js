...
function countAndCalculate() {
    ...
    for(let key in APP.cart) {
       const p = APP.cart[key]
       result.no_items += p.quantity
       result.base_price += p.price * p.quantity
       result.total_price += (p.price * p.quantity) * (1 + result.tax)
    }
    return result
}
...
function createCartItem(product) {
    ...
    btnDecrease.addEventListener("click", () => {
        ...
        if(product.quantity > 0) {
            ...
            APP.elements["pBasePrice"].innerHTML = `Price without tax: $${result.base_price.toFixed(2)}`
            APP.elements["pTax"].innerHTML = `Tax: ${(result.tax *100).toFixed(2)}%`
            APP.elements["pTotalPrice"].innerHTML = `Total: $${result.total_price.toFixed(2)}`
        }
        if(product.quantity == 0) {
            divCartItem.remove()
    ...
    `
    divCartItem.append(inputQuantity)
    inputQuantity.before(btnIncrease)
    inputQuantity.after(btnDecrease)
...
function createCartModal() {
    ...
    divCartContent.classList.add("cart_content")
    divOverlay.append(divCartContent)
    const btnCloseModal = document.createElement("button")
    btnCloseModal.innerHTML = `<i class="fas fa-xmark"></i>`
    btnCloseModal.addEventListener("click", () => {
        divOverlay.remove()
    })
    
    divCartContent.append(btnCloseModal)
    for(let key in APP.cart) {
        divCartContent.append(createCartItem(APP.cart[key]))
    }

    const pBasePrice = document.createElement("p")
    const pTax = document.createElement("p")
    const pTotalPrice = document.createElement("p")
    const divReceit = document.createElement("div")

    APP.elements["pBasePrice"] = pBasePrice
    APP.elements["pTax"] = pTax
    APP.elements["pTotalPrice"] = pTotalPrice
    APP.elements["divReceit"] = divReceit

    const result = countAndCalculate()
    pBasePrice.innerHTML = `Price without tax: $${result.base_price.toFixed(2)}`
    pTax.innerHTML = `Tax: ${(result.tax *100).toFixed(2)}%`
    pTotalPrice.innerHTML = `Total: $${result.total_price.toFixed(2)}`
...
window.addEventListener("load", () => {
    for(let product of PRODUCTS) {
        divProductList.append(createProductTile(product))
    }
})